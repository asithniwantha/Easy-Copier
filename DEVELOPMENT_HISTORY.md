...
- Resolved MVVM Toolkit warning MVVMTK0045 in `FileSystemItem.cs` and `GameDetailsViewModel.cs` by converting private backing fields (`_sizeFormatted`, `_folderStatusMessage`, `_isFolderStatusVisible`) to C# partial properties with `[ObservableProperty]` to ensure WinRT and AOT marshalling compatibility.
