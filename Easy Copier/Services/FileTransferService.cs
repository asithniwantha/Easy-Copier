...
namespace Easy_Copier.Services
{
    ...
    public class WindowsShellTransferService : IFileTransferService
    {
        ...
        public WindowsShellTransferService(
            ...
            ICopyHistoryService historyService)
        {
            _logger = logger;
            ...
            try
            {
                ...
                foreach (var game in request.Games)
                {
                    try
                    {
                        ...
                        await _historyService.LogAsync(new Models.CopyHistoryEntry
                        {
                            ItemName        = game.Name,
                            Category        = game.Category,
                            SourcePath      = game.FolderPath,
                            DestinationPath = destPath,
                            TargetDrive     = request.TargetDrive.DriveLetter,
                            BytesCopied     = result ? game.TotalBytes : 0,
                            Success         = result,
                            ErrorMessage    = result ? null : $"{game.Name}: Copy failed or was cancelled",
