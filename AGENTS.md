...
- **MVVMTK0045 Resolution**: Ensure that `[ObservableProperty]` is applied directly to `partial` properties rather than private backing fields. As the project uses C# preview (`<LangVersion>preview</LangVersion>`), this approach is natively supported and resolves the `MVVMTK0045` warnings generated by the CommunityToolkit.Mvvm source generators while avoiding the older `MVVMTK0041` conflicts.
